...
- `OrderSystem`：点单展示、订单评估、自动挂单、`F` 交付路由、完成配送、订单卡数据和状态文本。
...
- `StationInteractionSystem`：小费箱/钱箱、锅、仓库、餐具、垃圾桶、主食篮和漏勺的站点交互分发与提示文本；旧柜台、出餐口和应急采购入口在推车地图中保留节点但隐藏禁用。
...
- 玩家移动与站点交互：小费箱/钱箱、仓库、锅、锅内左右漏勺、主食篮、一次性餐具和垃圾桶。旧柜台、出餐口和应急采购方块在教程与第一关推车地图中隐藏禁用。

- 推车布局：教程和第一关共用新布局，包含前方顾客等待区、后方玩家操作区、中央大锅、锅内左右漏勺、后方库存/主食筐/餐具和右侧垃圾桶。

- 街道人流：上方街道支持路人和顾客从左右入口生成，并按配置比例进入等待区或穿过街道离开。

- 营业流程：开店、打烊、自动结束营业、收摊清理阶段、进入日结。

- 顾客流程：顾客生成、等待、自动挂单、耐心流失、吃完评价、到垃圾桶丢垃圾后离开、特殊顾客标记。

- 订单流程：查看订单、收银、即时交付、等待烹饪/配送、`F` 随地交付熟主食或通知熟配菜可取。

- 库存系统：熟食库存、生食库存、主食库存，以及统一的库存文本、缺货判断和补货落库。

- 烹饪系统：大锅批量煮配菜、主食漏勺、一次性餐具取熟主食规则、等待订单交付。

- 补货系统：普通供应商补货；缺货时可在仓库触发顾客改单或离开。旧应急采购代码仍保留给兼容入口，但当前推车地图不显示。
...
5. 等顾客来到推车前方自动点单，检查等待订单卡。

6. 测试即时订单、需要大锅配菜的订单、需要主食漏勺的订单。

7. 按 F 随地交付熟主食或通知顾客领取熟配菜。

8. 测试仓库普通补货，以及缺货时顾客改单或离开。

9. 打烊，等待清理条件满足，在小费箱/钱箱按 E 进入日结。
